<template>
    <div>
        <h1 style="text-align: center;">出库单</h1>
                    ...
                    <tr v-for="(row, i) in rows" :key="row.id">
                        <td v-for="(value, key) in row" :key="key">
                            ...
                            <div v-else-if="key == 'notes'">
                                <el-input v-model="row[key]" :placeholder="key" @keyup.enter="stopEditing(row, key)" />
                            </div>
                            <div v-else-if="key == 'price'">
                                <el-input :value="priceValue[i]" :placeholder="key" @keyup.enter="stopEditing(row, key)"
                                    :disabled="true" />
                            </div>
                            <el-input v-else v-model="row[key]" :placeholder="key" @keyup.enter="stopEditing(row, key)"
...
export default {
    ...
    setup() {
        ...
        const currentDateTimeDH = 'CK' + getCurrentDateTime();
        //表格序号
        const columns = ref([
            { id: 0, name: '序号' },
            { id: 1, name: '品名' },
        ...
        const confirmEvent = () => {
            var rowsrules = ""
            rows.value.forEach((element, index) => { // 遍历表格数据，将空值替换为0，避免计算错误。
                if (!element.kezhong) {
                ...
                element.price = priceValue.value[index]
